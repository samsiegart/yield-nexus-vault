...
const DepositInterface: React.FC<DepositInterfaceProps> = ({
  ...
  const [rebalanceMode, setRebalanceMode] = useState<"dollar" | "percentage">(
    "dollar"
  );
  ...
  const totalAllocated = selectedStrategies.reduce(
    (sum, s) => sum + (s.amount ? (s.amount / 100) * totalDepositAmount : 0),
    0
  );
          ...
          <TabsContent value="deposit" className="space-y-6">
            ...
            <Card className="bg-slate-800/60 border-slate-700">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <Label htmlFor="total-deposit" className="text-white">
                    Total Deposit Amount (USDC)
                  </Label>
                  <Input
                    id="total-deposit"
                    type="number"
                    ...
                    onChange={(e) =>
                      setTotalDepositAmount(Number(e.target.value))
                    }
          ...
          <TabsContent value="reallocate" className="space-y-6">
            ...
            <Card className="bg-slate-800/60 border-slate-700">
              <CardHeader>
                ...
                <CardDescription className="text-slate-300">
                  Set new target{" "}
                  {rebalanceMode === "dollar" ? "amounts" : "percentages"} for
                  your positions
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                  ...
                  currentPositions.map((position) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-slate-400">
                          {position.name} • {position.chain}
                        </div>
                        <div className="text-sm text-green-400">
                          {position.apy}% APY
                        </div>
                        <div className="text-sm text-slate-300">
                          Current: ${position.value.toFixed(2)}
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Input
